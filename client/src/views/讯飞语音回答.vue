<template>
  <div class="avatar">
    <div>
  <!-- 方式三：在表单元素中双向绑定 -->
  <input v-model="prompt" type="text" placeholder="输入内容...">
  <button @click="showRadarChart">发送</button>
  <div>
    <!-- 使用Vue的事件绑定 v-on:click 或 @click -->
    <button @click="convertText">准备好了</button>
    <button @click="playAudio" >开始面试</button>
  </div>
</div>
    <el-container>
...
export default {
  ...
  data() {
    return {
          // 声明缺失的属性并提供初始值
      volumePercent: 50, // 音量百分比，默认50%
      pitchPosition: 0, // 音调位置，默认0
        audioUrl : '', // 存储音频 URL
        prompt :"",
        chatText : '',
  ...
  methods: {
    convertText() {
            ...
            // 显示加载状态（可自行添加加载动
            document.querySelector('button').textContent = '准备好了';

            fetch('https://117.72.49.76/tts', {
                method: 'POST',
                headers: {
        ...
        playAudio() {
            ...
            if (this.audioUrl) {
                ...
                audio.play().catch(error => {
                    showError("播放失败，请点击重试或检查浏览器权限");
                    console.error("播放错误：", error);
                });
                return;
            }
                
            /* 方式 2：自动获取最新音频文件（如果 audioUrl 为空）
            this.getLatestAudioUrl().then(url => {
                if (url) {
                    this.audioUrl = url;
                    const audio = new Audio(this.audioUrl);
                    audio.play().catch(error => {
        ...
        async getLatestAudioUrl() {
            try {
                const response = await fetch('https://117.72.49.76/latest_audio');
                ...
                showError(`获取音频失败：${error.message}`);
                return null;
            }
        },
