<template>
  <div class="avatar">
    <div class="avatar" style="position: fixed; bottom: 200px; right: 20px; width: 400px; height: 80px; display: flex; align-items: center; gap: 10px; z-index: 100;">
  <div style="width: 40px; height: 40px; border-radius: 50%; background-color: #4a90e2; display: flex; align-items: center; justify-content: center; color: white; font-weight: bold;">U</div>
  <div style="display: flex; flex-direction: column; flex-grow: 1; height: 100%;">
    ...
    <input 
      ...
      type="text" 
      placeholder="输入内容..." 
      style="height: 40px; padding: 0 15px; border: 1px solid #ddd; border-radius: 20px; font-size: 16px; outline: none; transition: border-color 0.3s; box-shadow: 0 2px 8px rgba(0,0,0,0.08); width: 100%; box-sizing: border-box; margin-bottom: 8px;"
    >
    <button 
      @click="showRadarChart" 
      style="background-color: #4a90e2; color: white; border: none; border-radius: 20px; padding: 0 20px; margin: 0; font-size: 14px; cursor: pointer; transition: background-color 0.3s; display: inline-flex; align-items: center; justify-content: center; gap: 6px; height: 32px; float: right;"
    >
      <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-paper-plane" viewBox="0 0 16 16" style="margin-bottom: 2px;">
        <path d="M11.742 1.344a1.88 1.88 0 0 1 2.83 0l3.847 2.216a1 1 0 0 1 0 1.617l-8.128 4.625a1 1 0 0 1-.896 0L1.806 5.572a1 1 0 0 1 0-1.617l3.847-2.216a1.88 1.88 0 0 1 2.83 0z"/>
        <path d="M12 6.87v6.122c0 .597-.558.907-1.08.618l-8.09-4.757a1 1 0 0 1 0-1.236l8.09-4.757c.522-.289 1.08-.031 1.08.618z"/>
      </svg>
      发送
    </button>
  </div>
    ...
    async showRadarChart() {
            try {
                ...
                const response = await fetch('https://117.72.49.76:443/post_answer', {
                    ...
                    body: JSON.stringify({ user_id: 'test_user', question: this.prompt })
                });

                const data = await response.json();
                this.chatText = data.data || '';
                this.sendTextToAvatar(this.chatText);
